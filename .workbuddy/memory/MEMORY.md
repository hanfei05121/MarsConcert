...
- **播放模型 = 单视频 + 双音频**（feature/dual-audio-merge 分支）：
  - `video.mp4` 只承载画面（必须无音轨，`ffmpeg -an`），原唱/伴奏共用。
  - `orig.m4a` 原唱音频、`accomp.m4a` 伴奏音频（AAC）。
  - 播放窗用 `<video muted>`（只出画面，时间主源）+ 独立 `<audio>`（播 orig/accomp）；
    切原唱/伴奏只换 audio src 并对齐视频时钟，**视频不重新解码** → 解决旧双视频方案切换卡顿。
  - 音画同步：timeupdate 里音频漂移 >0.3s 拉回 `audio.currentTime = video.currentTime`。
- 兼容旧「双视频」素材：scanner 无 `video.mp4/orig.m4a` 时回退 `orig.mp4/accomp.mp4`（`<audio>` 可直接播其音轨）。
- 数据库 song 表：`orig_path` 为唯一键（新格式指向 orig.m4a），新增 `video_path` 列（旧库迁移时回填 orig_path）。
